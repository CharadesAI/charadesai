...
export default function Signin() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      type LoginResp = { data?: { user?: UserData; token?: string }; token?: string; user?: UserData; message?: string };
      const resp = await postJson<LoginResp>("/auth/login", { email: data.email, password_hash: passHash });
    ...
    <AuthLayout title="Welcome back" subtitle="Sign in to your account">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <Link to="/forgot" className="text-sm text-primary hover:underline">Forgot password?</Link>
          <Button type="submit" variant="hero" disabled={isSubmitting}>Sign in</Button>
        </div>
      </form>
