...
export function useProfile() {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ApiResponse<UserProfile>>("/api/auth/profile").then(
        (res) => res.data
      ),
...
export function useUpdateProfile() {
  ...
  return useMutation({
    mutationFn: (data: Partial<UserProfile>) =>
      fetchApi<ApiResponse<UserProfile>>("/api/auth/profile", {
        method: "PUT",
...
export function usePayments(page = 1) {
  return useQuery({
    ...
    queryFn: () =>
      ...
      >(`/api/payments?page=${page}`).then((res) => res.data),
    staleTime: 2 * 60 * 1000,
  });
}
...
export function useCurrentPlan() {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ApiResponse<CurrentPlan>>("/api/payments/current-plan").then(
        (res) => res.data
      ),
    staleTime: 5 * 60 * 1000,
  });
...
export function useUsageStats() {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ApiResponse<UsageStats>>("/api/usage/stats").then(
        (res) => res.data
      ),
...
export function useAIResults(limit = 10) {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ApiResponse<AIResult[]>>(`/api/ai/results?limit=${limit}`).then(
        (res) => res.data
      ),
...
export function useAIJobStatus(jobId: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchApi<ApiResponse<AIResult>>(`/api/ai/jobs/${jobId}`).then(
        (res) => res.data
      ),
    enabled: !!jobId,
